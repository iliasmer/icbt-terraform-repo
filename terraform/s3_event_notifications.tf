resource "aws_s3_bucket_notification" "s3_object_created_transcripts" {
  ...
  lambda_function {
    lambda_function_arn = module.pttsd_trigger_lambda.arn
    events = [
      "s3:ObjectCreated:Put",
      "s3:ObjectCreated:CompleteMultipartUpload"
    ]
  ...
  depends_on = [
    ...
    aws_lambda_permission.allow_s3_invoke_pttsd_trigger_lambda
  ]
}
...
resource "aws_lambda_permission" "allow_s3_invoke_pttsd_trigger_lambda" {
  statement_id  = "AllowS3InvokePTTSDTriggerLambda"
  action        = "lambda:InvokeFunction"
  function_name = module.pttsd_trigger_lambda.name
  principal     = "s3.amazonaws.com"
  source_arn    = module.s3_bucket_kth_thesis_documents.arn
}
